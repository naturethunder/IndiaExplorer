/**
 * icons.js — inline SVG icon set (Lucide-style: 24×24 viewBox, 2px stroke,
 * round caps/joins). Replaces every emoji icon in the UI so the whole site
 * shares one consistent, colorable icon language.
 *
 * Usage:  icon('mountain')                       → 24px stroke icon
 *         icon('star', { fill: true, size: 14 }) → filled 14px star
 *         icon('search', { cls: 'w-5 h-5' })     → sized via utility classes
 */
...
const ICONS = {
  ...
  landmark: '<path d="M3 22h18"/><path d="M6 18v-7"/><path d="M10 18v-7"/><path d="M14 18v-7"/><path d="M18 18v-7"/><path d="m4 7 8-5 8 5H4z"/>',
  ...
  temple: '<path d="M4 22h16"/><path d="M6 22V11"/><path d="M18 22V11"/><path d="M4 11h16"/><path d="m12 2 8 6H4l8-6z"/><path d="M10 22v-4a2 2 0 0 1 4 0v4"/>',
  tent: '<path d="M19 20 10 4"/><path d="m5 20 9-16"/><path d="M3 20h18"/><path d="m12 15-3 5"/><path d="m12 15 3 5"/>',
  compass: '<circle cx="12" cy="12" r="10"/><path d="m16.24 7.76-2.12 6.36-6.36 2.12 2.12-6.36 6.36-2.12z"/>',
  'map-pin': '<path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/>',
  map: '<path d="M9 3 3 5v16l6-2 6 2 6-2V3l-6 2-6-2z"/><path d="M9 3v14"/><path d="M15 5v14"/>',
  ...
  twitter: '<path d="M4 4l11.733 16H20L8.267 4z"/><path d="M4 20 10.768 12.232M13.228 11.768 20 4"/>',
 ...
 * opts.size   — px width/height (default 24; ignored if opts.cls sizes it)
 * opts.cls    — extra class(es) on the <svg> (e.g. 'w-5 h-5 text-primary')
 * opts.fill   — true → fill with currentColor (for star/heart badges),
 *               otherwise stroked outline (default)
 * opts.stroke — stroke width (default 2)
 * opts.label  — accessible label; when set, role="img" + <title>, else aria-hidden
 */
export function icon(name, opts = {}) {
  const body = ICONS[name];
  if (!body) return '';
  const size = opts.size || 24;
  const hasCls = !!opts.cls;
  const dims = hasCls ? '' : ' width="' + size + '" height="' + size + '"';
  const cls = hasCls ? ' class="' + opts.cls + '"' : '';
  ...
  return (
    '<svg xmlns="http://www.w3.org/2000/svg"' + dims + cls + a11y +
...
/** True if an icon name exists in the set. */
