...
export function destCardHTML(d, opts = {}) {
  ...
  return '' +
    ...
    '<img src="' + esc(cardImg(d)) + '" alt="' + esc((d.image && d.image.alt) || d.title) + '" class="card-img" loading="lazy" ' +
    'onerror="this.onerror=null;this.src=\'https://picsum.photos/seed/' + slug + '2/400/280\'" />' +
    '<div class="dest-card-overlay"></div>' +
    (d.badge ? '<div class="absolute top-3 left-3"><span class="badge bg-white/20 backdrop-blur-sm text-white border border-white/30 text-xs">' + esc(d.badge) + '</span></div>' : '') +
    typeBadge +
