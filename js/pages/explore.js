...
const typeIcon = new Map(DESTINATION_TYPES.map(function (type) {
  const iconName = CATEGORY_ICON_NAMES[type.id] || 'map-pin';
  return [type.id, icon(iconName, { size: 13, stroke: 1.8 })];
}));
...
function renderTypeButtons() {
  ...
  CATEGORY_FILTERS.forEach(function (t) {
    ...
    b.type = 'button';
    b.innerHTML = icon(CATEGORY_ICON_NAMES[t.id] || 'map-pin', { size: 16, stroke: 1.8 }) + '<span>' + esc(t.label) + '</span>';
    b.dataset.type = t.id;
...
function syncFilterActive() {
  Array.prototype.forEach.call(typeWrap.children, function (b) {
    const active = filters.type === (b.dataset.type || '');
    b.className = typeBtnClass(active);
    b.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
}

// ─── Core: filter, sort, render ────────────────────────
const PAGE_SIZE = 60;           // cards rendered per batch (avoids 2,361-node innerHTML on load)
let shown = PAGE_SIZE;
let lastResults = [];
...
function renderBatch() {
  ...
  // GSAP stagger reveal with scroll-triggered batches
  if (window.gsap && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    const cards = grid.querySelectorAll('.dest-card');
    // Immediate visible cards — stagger entrance
    ...
    // ScrollTrigger — reveal cards as they enter the viewport
  }
}
...
function apply() {
  ...
  if (filters.season) results = results.filter(function (d) { return (d.bestTime && d.bestTime.months) ? seasonsOf(d.bestTime.months).indexOf(filters.season) >= 0 : false; });
  if (filters.month) results = results.filter(function (d) { return (d.bestTime && d.bestTime.months) ? d.bestTime.months.includes(filters.month) : false; });
  // tiers[] is precomputed at build time: tiers a destination actually OFFERS
  // (a stay's price range overlaps the tier band).
  if (filters.tier) results = results.filter(function (d) { return d.tiers && d.tiers.includes(filters.tier); });
  ...
  const resultCountLive = document.getElementById('resultCountLive');
  if (resultCountLive) resultCountLive.textContent = results.length;
...
// Debounce the free-text search so we don't re-filter+re-render 2,361 rows on every keystroke.
...
function setDrawer(open) {
  ...
  document.getElementById('filterOpen').setAttribute('aria-expanded', open ? 'true' : 'false');
  if (open) document.getElementById('filterClose').focus();
}
...
if (window.gsap && !reducedMotion) {
  ...
  entrance
    .from('.explore-eyebrow', { opacity: 0, y: 18, duration: 0.55 })
    .from('.explore-hero h1', { opacity: 0, y: 42, duration: 0.9 }, '-=0.25')
    .from('.explore-intro', { opacity: 0, y: 22, duration: 0.6 }, '-=0.5')
    .from('.explore-stat-row > div', { opacity: 0, y: 20, duration: 0.55, stagger: 0.1 }, '-=0.35')
    .from('.explore-scroll-cue', { opacity: 0, duration: 0.5 }, '-=0.2');
  ...
  if (window.ScrollTrigger) {
    window.gsap.to('.explore-immersive-bg', {
      ...
      scale: 1.05,
      ease: 'none',
      scrollTrigger: { trigger: '.explore-hero', start: 'top top', end: 'bottom top', scrub: 0.8 },
    });
    window.gsap.from(['.category-strip-inner', '.filter-panel', '.results-toolbar'], {
      y: 32,
      opacity: 0,
      duration: 0.75,
      stagger: 0.12,
      ease: 'power3.out',
      scrollTrigger: { trigger: '.explore-content', start: 'top 82%', once: true },
    });
    window.ScrollTrigger.create({
      start: 40,
      onUpdate: function (self) {
        const nav = document.querySelector('.nav-glass');
        if (nav) nav.classList.toggle('nav-scrolled', self.scroll() > 40);
      },
    });
  }
}
