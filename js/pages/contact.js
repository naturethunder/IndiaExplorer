...
function clearErrors() {
  ...
  err.textContent = '';
  // Remove aria-describedby from all inputs when errors are cleared
  Object.values(inputs).forEach(input => {
    if (input) input.removeAttribute('aria-describedby');
  });
}

function showErr(msg, focusEl, fieldName) {
  err.textContent = msg;
  err.style.display = 'block';
  // Associate the error with the specific field for screen readers
  if (fieldName && inputs[fieldName]) {
    inputs[fieldName].setAttribute('aria-describedby', 'cf-error');
  }
...
form.addEventListener('submit', function (e) {
  ...
  if (!name) { showErr('Please enter your name.', inputs.name, 'name'); return; }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showErr('Please enter a valid email address.', inputs.email, 'email'); return; }
  if (!message) { showErr('Please enter a message.', inputs.message, 'message'); return; }
