...
(function () {
  ...
  el.innerHTML = cats.map((c) => {
    ...
    return '<a href="' + href + '" class="category-chip">' +
      '<span class="category-chip-icon ' + c.tint + '">' + icon(c.ic, { size: 32 }) + badge + '</span>' +
...
// Replaces the IntersectionObserver + setTimeout approach.
// Cards get a spring stagger; headings get a subtle fade-up.
// All calls no-op when prefers-reduced-motion is active.
// The CSS .reveal / .in-view fallback is preserved for no-JS.
(async function () {
  const { revealFadeUp, revealStaggerChildren, revealParallaxBg } =
    await import('../utils/gsap-animations.js').catch(() => ({}));

  // If the module failed to load (e.g. GSAP vendor unavailable),
  // gsap-animations.js already called showAllInstantly() — nothing to do.
  if (!revealFadeUp) return;

  // ── Section headings / text blocks ────────────────────────
  // Subtle fade-up (power1.out, 350ms, y:16) — per ui-ux-pro-max Subtle tier.
  // Every .reveal container gets a fade-up, including those that hold card grids.
  // The container fades in as a whole; the grid children inside also get their own
  // independent stagger animation. The gsap-managed guard prevents double-animation.
  document.querySelectorAll('.reveal').forEach((el) => revealFadeUp(el));

  // ── Card grids — stagger children ─────────────────────────
  // Spring stagger (back.out(1.4), 400ms, 0.06s each, scale 0.94 + y:20)
  // per ui-ux-pro-max Standard Stagger List tier.
  // Each grid is wired AFTER JS renders its cards (microtask delay).
  await Promise.resolve(); // yield so card-rendering IIFEs finish first

  [
    { id: 'popular-grid',  each: 0.06 },
    { id: 'hills-grid',    each: 0.06 },
    { id: 'explore-grid',  each: 0.05, maxChildren: 6 },
    { id: 'budget-grid',   each: 0.07, maxChildren: 5 },
    { id: 'season-grid',   each: 0.08, maxChildren: 4 },
    { id: 'trending-grid', each: 0.05, maxChildren: 8, scale: 0.96, y: 14 },
    { id: 'month-rail',    each: 0.07, maxChildren: 4 },
  ].forEach(({ id, each, maxChildren, scale, y }) => {
  ...
  // Subtle (yPercent: 8, scrub: true) — bg-only, never on text/controls.
  // Per ui-ux-pro-max Parallax Scroll / Subtle tier.
  const heroBg = document.getElementById('heroBg');
  const heroSection = heroBg?.closest('.hero-home');
  if (heroBg && heroSection) {
    revealParallaxBg(heroBg, heroSection, { yPercent: 8 });
  }
})();
