...
function main(dest, idx) {
  ...
  (function heroCarousel() {
    ...
    if (!galleryUrls.length) galleryUrls = ['images/kanatal-immersive-bg.png'];
    build(galleryUrls);
  })();
  ...
  function renderOverview() {
    ...
    const topPlaces = places.slice(0, 4).map(function (p, i) {
      ...
      return '<div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-orange-50 hover:border-orange-200 transition-all" data-topidx="' + i + '" role="button" tabindex="0">' +
        (pImgSrc ? '<img src="' + esc(pImgSrc) + '" alt="' + esc(pImgAlt) + '" class="w-14 h-12 rounded-lg object-cover shrink-0" loading="lazy" onerror="this.onerror=null;this.style.display=\'none\';" />' : '') +
        '<div class="min-w-0">' +
        ...
        '<p class="text-xs text-amber-600 font-medium">★ ' + esc(String(p.rating || '4.5')) + '</p>' +
        '</div></div>';
    }).join('');
    ...
    const gemCards = gems.map(function (g) {
      ...
      const gImgSrc = (typeof p.image === 'string' ? p.image : (p.image && p.image.src ? p.image.src : '')) || '';
      const gImgAlt = (p.image && p.image.alt) ? p.image.alt : (p.name || '');
      return '<div class="card p-0 overflow-hidden cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all" data-uridx="' + g.idx + '" role="button" tabindex="0">' +
        '<div class="relative">' +
        (gImgSrc ? '<img src="' + esc(gImgSrc) + '" alt="' + esc(gImgAlt) + '" class="w-full h-28 object-cover" loading="lazy" onerror="this.onerror=null;this.style.display=\'none\';" />' : '<div class="w-full h-28 bg-gray-200"></div>') +
        ...
        '<span class="text-xs text-amber-600 font-medium shrink-0">★ ' + esc(String(p.rating || '4.5')) + '</span></div>' +
    ...
    function get5RealPhotos() {
      ...
      if (typeof dest.heroImage === 'string' && dest.heroImage) {
        photos.push({ src: dest.heroImage, title: dest.title + ' — ' + (dest.tagline || dest.state), subtitle: dest.state + ' · Main View', category: dest.type });
    ...
    if (list.length === 0) {
      ...
      cards = list.map(function (s) {
        ...
        return '<div class="card p-0"><div class="flex flex-col sm:flex-row">' +
          ...
          '<img src="' + esc(s.image.src) + '" alt="' + esc(s.image.alt || s.name) + '" class="w-full h-44 sm:h-full object-cover hover:scale-105 transition-transform" loading="lazy" onerror="this.onerror=null;this.style.display=\'none\';" />' +
  ...
  document.getElementById('similar-grid').innerHTML = similar.map(function (d) {
    return '<a href="' + destUrl(d.slug) + '" class="group block rounded-2xl p-3 border border-white/20 bg-slate-900/90 backdrop-blur-md shadow-xl hover:border-emerald-400/60 hover:-translate-y-1 transition-all" style="background: rgba(15, 23, 42, 0.90) !important;">' +
      '<div class="rounded-xl overflow-hidden aspect-video relative mb-2.5">' +
      '<img src="' + esc(d.image.src) + '" alt="' + esc(d.image.alt) + '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" onerror="this.onerror=null;this.style.display=\'none\';" />' +
      '<div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>' +
      '<div class="absolute bottom-2 left-2 right-2"><p class="text-white text-xs font-bold truncate" style="color: #ffffff !important;">' + esc(d.title) + '</p><p class="text-white/80 text-xs" style="color: rgba(255, 255, 255, 0.8) !important;">' + esc(d.state) + '</p></div>' +
      '</div>' +
      '<p class="font-bold text-sm text-white truncate" style="color: #ffffff !important; font-weight: 700 !important;">' + esc(d.title) + '</p>' +
      '<p class="text-xs font-bold text-emerald-400 mt-0.5" style="color: #34d399 !important; font-weight: 700 !important;">From ₹' + inr(d.minPrice) + '/night</p></a>';
  }).join('');
  ...
  function fetchPlacePhotos(p, cb) {
    if (Array.isArray(p.photos) && p.photos.length) {
      const valid = p.photos.filter(u => u && typeof u === 'string' && !u.includes('picsum.photos'));
      if (valid.length > 0) {
        cb(valid);
        return;
      }
