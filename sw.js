...
self.addEventListener('fetch', (event) => {
  ...
  if (req.destination === 'image' || /\.(png|jpg|jpeg|webp|gif|svg)(\?.*)?$/i.test(url.pathname)) {
    // ─── CRITICAL: Never intercept external images (Wikimedia, Unsplash, Pexels, Flickr etc.)
    // Let the browser handle all cross-origin image requests natively to avoid blank-pixel failures.
    ...
    event.respondWith(
      caches.match(req).then((cached) => {
        ...
        return fetch(req)
          .then((networkRes) => {
            // Cache same-origin local assets only
          ...
          .catch(() => {
            // Only serve 1x1 PNG when genuinely offline for same-origin assets
            if (!self.navigator.onLine) {
              return new Response(EMPTY_PNG, {
                headers: { 'Content-Type': 'image/png' }
              });
            }
            // Online: propagate error so browser onerror fallbacks trigger
