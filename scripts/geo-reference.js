...
const CITIES = [
  ['Delhi', 28.6139, 77.2090, 'Delhi UT'],
  ...
  ['Srinagar', 34.0837, 74.7973, 'Jammu & Kashmir UT'],
  ['Jammu', 32.7266, 74.8570, 'Jammu & Kashmir UT'],
  ...
  ['Leh', 34.1526, 77.5771, 'Ladakh UT'],
  ['Puducherry', 11.9416, 79.8083, 'Puducherry UT'],
  ['Port Blair', 11.6234, 92.7265, 'Andaman & Nicobar UT'],
  ['Kavaratti', 10.5593, 72.6358, 'Lakshadweep UT'],
  ['Daman', 20.3974, 72.8328, 'Daman & Diu UT']
];

function nearest(coord, list) {
  let best = null, bestD = Infinity;
  for (const row of list) {
    const d = haversineKm(coord, [row[1], row[2]]);
...
/**
 * Distance-from-major-cities rows for the reach table, covering all 36 States & UTs.
 * Returns the nearest representative city per State/UT (36 routes total), sorted by distance.
 *   → [{ from, city, state, distance, byCar, byTrain, byAir, via }]
 */
function majorCityRoutes(lat, lng, name, limit) {
  ...
  CITIES.forEach((c) => {
    ...
    if (km > 5) {
      if (!byState.has(state) || km < byState.get(state).km) {
        byState.set(state, { city, state, km: Math.round(km) });
      }
    }
  });

  const sorted = Array.from(byState.values()).sort((a, b) => a.km - b.km);
  const sliced = (limit && limit < sorted.length) ? sorted.slice(0, limit) : sorted;
