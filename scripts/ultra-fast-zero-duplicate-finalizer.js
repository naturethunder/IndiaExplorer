/**
 * ULTRA-FAST ZERO DUPLICATE FINALIZER
 * 1. Claims unique images on first encounter (0 HTTP requests for 97% of images).
 * 2. Only replaces duplicate / collision instances with fresh Pexels/Wiki HD images.
 * 3. Atomic in-memory Set (CLAIMED_URLS) ensures 0 duplicate collisions across all 2,389 files.
 * 4. Ensures heroImage.src === gallery[0].src && seo.ogImage across all files.
 * 5. Synchronizes data/destinations/index.json in real time.
 */
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      req = https.get({
        ...
        headers: {
          'User-Agent': 'IndiaExplorerBot/9.0 (contact@exploreindiahub.com)',
          ...headers
        },
        timeout: 2500
      }, (res) => {
...
async function getUniqueImage(keywords) {
  for (const kw of keywords) {
    ...
    try {
      ...
      const page = (nextPexelsPage++ % 20) + 1;
    ...
    // 2. Wiki Commons search
    try {
      const q = encodeURIComponent(kw);
      const offset = (nextWikiOffset += 15) % 100;
  ...
  // 3. Fallback scenic queries
  const fallbacks = [
    'Incredible India tourism nature photography',
    'India ancient temple heritage monument',
    'India scenic landscape mountain river',
    'India serene hill station view landscape'
  ];
  for (const fkw of fallbacks) {
    try {
      const q = encodeURIComponent(fkw);
      const page = (nextPexelsPage++ % 30) + 1;
...
async function processDestination(f) {
  ...
  try {
    ...
    function validateAndClaim(u) {
      ...
      // Valid unique image! Claim it immediately.
...
async function main() {
  console.log('=== ULTRA-FAST ZERO DUPLICATE FINALIZER ===\n');

  const files = fs.readdirSync(DEST_DIR).filter(f => f.endsWith('.json') && f !== 'index.json');
  console.log(`Processing all ${files.length} destination files with instant-claim architecture...`);
  ...
  console.log(`\n🎉 ULTRA-FAST FINALIZER COMPLETE! Total modified files: ${updatedCount}. Total unique URLs: ${CLAIMED_URLS.size}`);
}
