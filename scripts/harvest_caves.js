/**
 * HARVEST CAVES CATEGORIES (200+ FRESH UNUSED HD IMAGES)
 */
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/220.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function harvest() {
  console.log('=== HARVESTING CAVES CATEGORIES ===\n');
  ...
  fs.writeFileSync('scripts/caves_pool.json', JSON.stringify(pool, null, 2), 'utf8');
  console.log(`\n🎉 SAVED ${pool.length} FRESH IMAGES TO scripts/caves_pool.json`);
}
