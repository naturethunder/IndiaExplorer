/**
 * HARVEST ARCHAEOLOGY & HERITAGE (2,000+ FRESH UNUSED HD IMAGES)
 */
...
const categories = [
  ...
  'Category:Archaeological sites in Ladakh',
  'Category:Archaeological sites in Himachal Pradesh',
  'Category:Archaeological sites in Uttarakhand',
  'Category:Archaeological sites in Assam'
];
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/100.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function harvest() {
  console.log('=== HARVESTING ARCHAEOLOGY & HERITAGE POOL ===\n');
  ...
  fs.writeFileSync('scripts/archaeology_pool.json', JSON.stringify(pool, null, 2), 'utf8');
  console.log(`\n🎉 SAVED ${pool.length} FRESH IMAGES TO scripts/archaeology_pool.json`);
}
