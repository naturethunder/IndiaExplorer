/**
 * HARVEST WATER & BRIDGES CATEGORIES (300+ FRESH UNUSED HD IMAGES)
 */
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/210.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function harvest() {
  console.log('=== HARVESTING WATER & BRIDGES CATEGORIES ===\n');
  ...
  fs.writeFileSync('scripts/dams_bridges_pool.json', JSON.stringify(pool, null, 2), 'utf8');
  console.log(`\n🎉 SAVED ${pool.length} FRESH IMAGES TO scripts/dams_bridges_pool.json`);
}
