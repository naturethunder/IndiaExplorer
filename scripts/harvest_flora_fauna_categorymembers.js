/**
 * HARVEST FLORA & FAUNA STATE CATEGORIES (1,000+ FRESH UNUSED HD IMAGES)
 */
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/160.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function harvest() {
  console.log('=== HARVESTING FLORA & FAUNA STATE CATEGORIES ===\n');
  ...
  fs.writeFileSync('scripts/flora_fauna_pool.json', JSON.stringify(pool, null, 2), 'utf8');
  console.log(`\n🎉 SAVED ${pool.length} FRESH IMAGES TO scripts/flora_fauna_pool.json`);
}
