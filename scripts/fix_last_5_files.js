/**
 * FIX LAST 5 FILES TO ACHIEVE 100% ZERO COLLISIONS REPOSITORY-WIDE
 */
...
// Gather claimed URLs from all OTHER files (excluding the 5 targets)
const targets = new Set([
  'yeshwantgad.json',
  'yoga-madhava-temple-settikere.json',
  'yoganarasimha.json',
  'zangla-khar.json',
  'zanskar-valley.json'
]);
...
console.log(`Other 2,384 files have ${claimed.size} unique URLs claimed.`);

const queries = [
  'Redi fort Sindhudurg Yeshwantgad',
  'Settikere Yoga Madhava temple Hoysala',
  'Devarayanadurga Yoganarasimha temple hill',
  'Zangla palace Zanskar castle Ladakh',
  'Zanskar river gorge mountain valley',
  'Padum Zanskar landscape monastery',
  'Karsha gompa Zanskar cliff view',
  'Phugtal monastery Zanskar gorge cliff',
  'Suru valley Nun Kun glacier',
  'Drass war memorial valley Ladakh'
];
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/300.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function run() {
  console.log('=== HARVESTING FRESH UNIQUE IMAGES FOR LAST 5 FILES ===\n');
  ...
  console.log(`Harvested ${pool.length} 100% fresh never-before-used images.`);
  ...
  console.log('\n=== FINAL COMPREHENSIVE MATHEMATICAL AUDIT ===');
  let tajViolations = 0;
