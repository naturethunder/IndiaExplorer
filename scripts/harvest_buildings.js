/**
 * HARVEST BUILDINGS & STRUCTURES (400+ FRESH UNUSED HD IMAGES)
 */
...
const categories = [
  ...
  'Category:Buildings and structures in Gujarat',
...
function fetchJson(url) {
  return new Promise((resolve) => {
    ...
    try {
      req = https.get(url, {
        headers: {
          'User-Agent': 'IndiaExplorerBot/180.0 (contact@exploreindiahub.com)'
        }
      }, (res) => {
...
async function harvest() {
  console.log('=== HARVESTING BUILDINGS & STRUCTURES CATEGORIES ===\n');
  ...
  fs.writeFileSync('scripts/buildings_pool.json', JSON.stringify(pool, null, 2), 'utf8');
  console.log(`\n🎉 SAVED ${pool.length} FRESH IMAGES TO scripts/buildings_pool.json`);
}
