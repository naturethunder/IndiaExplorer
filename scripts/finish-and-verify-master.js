...
async function finish() {
  console.log('=== FINAL COMPLETION & REPOSITORY SYNCHRONIZATION ===\n');

  const files = fs.readdirSync(DEST_DIR).filter(f => f.endsWith('.json') && f !== 'index.json');
  console.log(`Auditing remaining destinations across ${files.length} total files...`);

  let updated = 0;
  for (let i = 0; i < files.length; i++) {
    const isUp = await processFile(files[i]);
    if (isUp) updated++;
    if ((i + 1) % 200 === 0 || i + 1 === files.length) {
      console.log(`Progress: ${i + 1}/${files.length} (${updated} files updated, ${GLOBAL_USED_URLS.size} global unique URLs)`);
    }
  }
  ...
  console.log(`\n🎉 MASTER REPOSITORY-WIDE STRICT REPAIR & VERIFICATION 100% COMPLETE!`);
  console.log(`Total Global Unique URLs: ${GLOBAL_USED_URLS.size}`);
}
