/**
 * PARALLEL SWEEP CLOSER WORKER
 * Sweeps a specific range of destination files with guaranteed 5-gallery and 3-place photo fills,
 * zero duplicate URLs, and zero generic stock fillers.
 */
...
async function main() {
  ...
  const endIdx = parseInt(process.argv[3] || '2389', 10);
  const agentName = process.argv[4] || 'Worker';

  console.log(`========================================================================`);
  console.log(`  [${agentName}] PARALLEL SWEEP WORKER (Indices ${startIdx} to ${endIdx}) `);
  console.log(`========================================================================\n`);

  initGlobalIndex();

  const allFiles = fs.readdirSync(DEST_DIR).filter(f => f.endsWith('.json') && f !== 'index.json');
  const myFiles = allFiles.slice(startIdx, endIdx);

  console.log(`[${agentName}] Processing ${myFiles.length} assigned destination files...`);
  ...
  for (let b = 0; b < myFiles.length; b += BATCH_SIZE) {
    ...
    if (processed % 32 === 0 || processed === myFiles.length) {
      console.log(`[${agentName}] Progress: ${processed} / ${myFiles.length} destinations (${modifiedCount} updated).`);
    }
  }

  console.log(`\n🎉 [${agentName}] COMPLETE: Processed ${myFiles.length} files (${modifiedCount} updated).`);
}
