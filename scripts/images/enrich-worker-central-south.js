/**
 * PARALLEL IMAGE ENRICHMENT WORKER: Central, South & North-East
 * Target States: Telangana, Tripura
 * 
 * Rules Enforced:
...
const TARGET_STATES = ['Telangana', 'Tripura'];
...
async function runWorkerCentralSouth() {
  ...
  console.log(`========================================================================`);
  console.log(`  WORKER CENTRAL-SOUTH: Processing Telangana, Tripura`);
  ...
  for (const summary of index.destinations) {
    ...
    count++;
    console.log(`\n[Worker Central-South #${count}] Enriching: ${d.title} (${d.slug}) [State: ${stateName}]`);
  ...
  await cache.close();
  console.log(`\n=== WORKER CENTRAL-SOUTH FINISHED (${count} destinations enriched) ===\n`);
}

if (require.main === module) {
  runWorkerCentralSouth().catch(console.error);
}

module.exports = { runWorkerCentralSouth };
