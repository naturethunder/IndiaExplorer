...
async function runFinish() {
  console.log(`Starting final polish across ${target16.length} files...`);

  for (const filename of target16) {
    ...
    // 1. If gallery count < 5, pad with unique photo
    ...
    for (const slot of allSlots) {
      ...
      if (isDefective(slot) || claimedNow.has(k)) {
        ...
        console.log(`  [${filename}] Fixing ${slot.type} (${placeName})...`);
        const queries = [
          `${placeName} ${state}`,
          `${placeName} temple landmark`,
          `${title} ${state}`,
    ...
    if (d.gallery && d.gallery.length < 5) {
      console.log(`  [${filename}] Padding gallery from ${d.gallery.length} to 5...`);
      while (d.gallery.length < 5) {
        const queries = [`${title} ${state}`, `${state} ancient temple`, `${state} heritage vista`];
        const rep = await getPhoto(queries, state, usedKeys);
        if (rep) {
          d.gallery.push({
            src: rep.url,
