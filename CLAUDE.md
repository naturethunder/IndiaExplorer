# IndiaExplore — Project Guide (updated 2026-08-01)
...
An India travel-discovery website: browse **2,389 destinations** (13,991 places,
9,764 stays across 36 states/UTs), filter by type/budget/state/month, view per-destination
...
> **Hotel enrichment handoff (verified 2026-08-01).** `scripts/bulk/fill-real-hotels.js` completed
> one resumable pass across all 2,389 canonical destinations. Every detail JSON has
> `hotelSourceTried: true`; 60 rate-limited destinations also have `hotelSourceError: true` and are
> the only intended retry set. Current totals: 7,570 OSM-sourced replacements recorded, 2,085 exact
> generated-template names remaining, and 9,150/9,764 hotel images still on picsum. The script may
> update only `hotels[].name`, best-effort `hotels[].image`, and hotel source markers in
> `data/destinations/*.json`; never run `build-json-data.js` afterward. Verified with
> `node scripts/validate-filters.js` (2,389 destinations / 36 states) and
> `node scripts/verify-picsum-photos.js` (154 hero / 2,954 place placeholders).
...
> `home.js`, `finder.js`, and data layers. If a place has 3 real photos, it renders strictly 3 real photos
> (1/3, 2/3, 3/3). No 4th/5th fake image padding or stock placeholder is appended. An automated & manual audit
> filtered out all PDF document scans, magazine articles, insect diagrams, and dictionary entry images from Wikimedia queries.
