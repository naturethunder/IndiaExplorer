...
1. **One reusable detail template.** `destination.html?slug=<slug>` renders *any* destination.
   Never create one HTML file per destination. The 2,355 redirect stubs (`destination.html?slug=<slug>`) are stored neatly inside the `stubs/` directory (`stubs/<slug>.html`), regenerated by `scripts/build-stubs.js`, keeping the root workspace directory clean while ensuring old links / bookmarks keep working.
