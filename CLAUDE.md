# IndiaExplore — Project Guide (updated 2026-08-14)
...
An India travel-discovery website: browse **2,389 destinations** (14,001 places,
...
frosted glass panels, fixed cinematic background images, and emerald accent glows.
...
> were **hand-added & initially enriched** via `scripts/enrich-new-destinations-full.js` +
> `scripts/fetch-verified-wikimedia-photos.js`. Treat the media-restoration handoff below as the
> current authority for image coverage and authenticity.
> The remaining ~2,247 were **bulk-ingested** from Wikidata + Wikipedia (`scripts/bulk/`) across 36 state/UTs.

> **Media restoration handoff (2026-08-14; supersedes earlier coverage claims).**
> - Preserved **2,389 destinations / 14,001 places / 9,756 stays**; no catalog records were deleted.
> - At the owner's request, restored missing media fields from the pre-cleanup backup for
>   **89 destination covers / 4,196 places / 9,425 stays**. Primary URL coverage is now
>   **2,389 / 2,389 destinations, 14,001 / 14,001 places, and 9,756 / 9,756 stays**.
> - The six verified online replacements for Havelock Island, Vodarevu Beach, Renuka Sanctuary,
>   Gulf of Mannar Marine National Park, Kelve Fort, and Girnar Wildlife Sanctuary were preserved.
> - Restored legacy coverage is **not equivalent to verified entity-accurate photography**. It may
>   include generic stock, document/scientific scans, low-resolution media, or stale URLs and must
>   be re-audited before any `100% verified real` claim is made.
> - Pre-cleanup source: `scripts/backups/media-repair-2026-08-14.json.gz`. Reversible pre-restore
>   snapshot: `scripts/backups/media-before-old-data-restore-2026-08-14.json.gz`.
>
> **Previous hotel and hero enrichment (2026-08-03; historical, not a current authenticity claim).**
> - **Destination Hero URL Coverage:** **2,389 / 2,389** after the 2026-08-14 legacy restoration.
...
> **Media authenticity policy.**
> Do not infer authenticity from URL coverage. Newly sourced media must be legal, online-hosted,
> high-resolution, and matched to the exact destination/place/stay. Do not download new image files
> into the repository. Legacy-restored fields remain an explicit audit backlog.
...
- **No fake / dummy stock fallbacks.** Never use `picsum.photos`, generic stock, or a destination
  photo for a different place/stay. Show `Photo unavailable` until an exact real photo is verified.
- **Do not run legacy media scripts blindly.** Several older fetch/assignment scripts contain
  generic or destination-level fallback paths. Use `repair-media-integrity.js` for validation and
  the strict exact-match fetchers for recovery.
- Exact stay recovery requires both exact property-title equality and accommodation context.
  Location scenery, landmarks, and language pages must never be assigned as stay photos.
...
| Page | Module | Role |
|---|---|---|
| `index.html` | `js/pages/home.js` | Home: hero (fixed full-bleed image bg + search + autocomplete + social-proof avatars), 8-chip category strip, interactive "Explore India" SVG map + "Best This Month" rail, trending/season/budget/hills/explore grids. Loads only `index.json` (+ lazy-imports `indiaMap.js`). **Uses dark glassmorphism theme** (`glass-immersive` body class + bg/overlay divs). |
