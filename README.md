...
A India travel-discovery website. Browse **2,389 destinations** (14,001 places,
9,756 stays across 36 states & UTs), filter by type / budget / state / travel-month, and
...
> **Media status (restored 2026-08-14).** Missing media fields were restored from the old catalog
> backup without deleting records: URL coverage is now 2,389/2,389 destinations, 14,001/14,001
> places, and 9,756/9,756 stays. Six newly verified online destination replacements were preserved.
> Restored legacy URLs are not all authenticity-verified and remain an audit backlog. See
> [CLAUDE.md](CLAUDE.md) and [docs/AUDIT.md](docs/AUDIT.md) for the exact boundary.
...
- **One reusable detail template.** `destination.html?slug=goa` renders *any* destination.
  There is never one HTML file per destination — the 2,389 `<slug>.html` files are ~10-line
...
├── destination.html        # ⭐ The ONE real detail page (all 2,389 render via ?slug=)
├── about / privacy / terms / contact.html   # Company pages (contact form, no backend)
├── stubs/
│   └── <slug>.html  (×2383)  # Redirect stubs → destination.html?slug=<slug>
│
├── css/
...
- **Online image hosts** — destination JSON stores direct Wikimedia Commons, Unsplash, Pexels,
  and legacy source URLs; the browser does not download image files into the repository. URL
  coverage is complete, but legacy-restored media remains subject to identity and quality review.
