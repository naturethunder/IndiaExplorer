...
A India travel-discovery website. Browse **2,383 destinations** (13,958+ places,
9,629+ stays across 36 states & UTs), filter by type / budget / state / travel-month, and
...
> **108 curated + 28 offbeat enriched + ~2,247 bulk-ingested.** Hand-authored with baked real
> Wikimedia photos (no picsum/dummy stock fallbacks). See [CLAUDE.md](CLAUDE.md) for provenance.
...
- **One reusable detail template.** `destination.html?slug=goa` renders *any* destination.
  There is never one HTML file per destination — the 2,383 `<slug>.html` files are ~10-line
...
├── destination.html        # ⭐ The ONE real detail page (all 2,383 render via ?slug=, featuring 5-real-image overview carousel)
...
│   ├── destinations/
│   │   ├── index.json      # Light manifest: 2,383 summaries + filter meta (tiers/types/states/months)
│   │   └── <slug>.json  (×2383)  # Full per-destination detail (schema below)
...
│   ├── build-stubs.js      # Regenerates the 2,383 redirect stubs
...
node scripts/build-stubs.js       # rebuild the 2,383 redirect stubs
```
