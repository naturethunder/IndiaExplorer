...
> **Platform Status (2026-09-20 rev-18):** **2,393 destinations** (14,013 places, 10,427 verified authentic stays across all 36 states & UTs). 100% zero-duplicate, landmark-verified photography with **66,700+ globally unique image URLs**. **Phase 55: Master Elimination of Cross-Monument Mislabeling & Rule 0 Enforcement —** Programmatically validated photographic truth ([.agents/rules/destination-strict-rules.md](.agents/rules/destination-strict-rules.md)), 0 mislabeled monuments, 0 foreign stock, 100% zero-human/zero-portrait verification, 20 scraped news tragedies sanitized to authentic attractions, complete mobile/desktop responsiveness, and dual-engine light/dark mode parity. Automated UI/UX QA audit: **0 issues detected (100/100 score)**.

> **Image Pipeline Status (2026-09-20 rev-18):** **Phase 55 Master Purification Complete —** 100% True HD (1920px+) photography assigned across the entire catalog (100% live HTTP 200 OK, 100% unique, zero cross-monument borrowing, zero human faces, zero devotee mobs, zero foreign stock, zero Wikimedia, zero rate limits). Total catalog: **66,700+ verified HD photos**.
...
> **100% Verified Legal Photography & Rule 0 Photographic Truth.** Hand-authored and enriched with authentic
> Pexels API, Unsplash, and Openverse/Flickr CDN photography as primary sources (zero picsum/PDF/dummy stock fallbacks, zero foreign stock from outside India, zero portraits/selfies/vehicles/power-lines, zero internal or cross-destination duplicates). All stock photography is vetted against the **Rule 0 Metadata Mismatch Matrix** ([.agents/rules/destination-strict-rules.md](.agents/rules/destination-strict-rules.md)) to ensure zero cross-monument borrowing or mislabeling. Wikimedia Commons is fully removed from all overhauled destinations (Phases 29–55 extended this catalog-wide). See [CLAUDE.md](CLAUDE.md) and [.agents/rules/destination-strict-rules.md](.agents/rules/destination-strict-rules.md) for provenance.
...
- **Rule 0 Photographic Truth & Multi-Provider Pipeline** — High-resolution verified photography baked into destination JSONs (`heroImage`, `gallery`, `topPlaces[].photos`, `hotels[].image`). Enforces **Rule 0 (Strict Photographic Truth)**: zero cross-monument borrowing (e.g. no Kumbhalgarh for Maharashtra forts, no foreign castles/churches) per the metadata mismatch matrix in [.agents/rules/destination-strict-rules.md](.agents/rules/destination-strict-rules.md), zero-collision detection across 66k+ repo URLs, and strict banned-pattern filtering (no portraits, crowds, vehicles, foreign monuments).
- **Ground-Truth Archives & CC CDNs** — Wikimedia Commons 4K/5K archives, Panoramio ground-truth datasets, Flickr CC, Pexels API, and Unsplash HD CDN. Direct Wikimedia hotlinks are strictly avoided or proxied to prevent CDN HTTP 429 rate-limiting.
